...
const out = path.resolve("docs/reports/pixel-cat-preview.png");
const frameSize = 128;
const cellSize = 144;
const padding = 8;
const rows = [
  ["static.png", "aod.png"],
  Array.from({ length: 8 }, (_, index) => `wake_${index}.png`),
  Array.from({ length: 8 }, (_, index) => `tap_${index}.png`),
  Array.from({ length: 12 }, (_, index) => `feed_${index}.png`),
  Array.from({ length: 10 }, (_, index) => `happy_${index}.png`),
  Array.from({ length: 8 }, (_, index) => `no_food_${index}.png`)
];
...
const width = Math.max(...rows.map((row) => row.length)) * cellSize + padding * 2;
const height = rows.length * cellSize + padding * 2;
...
for (let rowIndex = 0; rowIndex < rows.length; rowIndex += 1) {
  for (let colIndex = 0; colIndex < rows[rowIndex].length; colIndex += 1) {
    const image = await readPng(rows[rowIndex][colIndex]);
...
console.log(`rendered sprite preview: ${out}`);
