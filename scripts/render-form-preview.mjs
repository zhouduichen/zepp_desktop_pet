...
const formIds = ["baby", "teen", "active", "steady", "explorer", "rare", "secret"];
const root = path.resolve("pet-packs");
const out = path.resolve("docs/reports/starter-form-preview.png");
const frameSize = 128;
const cellSize = 144;
const padding = 8;

async function readPng(petId, formId) {
  const buffer = await readFile(path.join(root, petId, formId, "static.png"));
...
const width = formIds.length * cellSize + padding * 2;
...
for (let rowIndex = 0; rowIndex < petIds.length; rowIndex += 1) {
  for (let colIndex = 0; colIndex < formIds.length; colIndex += 1) {
    const image = await readPng(petIds[rowIndex], formIds[colIndex]);
...
console.log(`rendered starter form preview: ${out}`);
